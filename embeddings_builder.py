...
FAISS vector index + metadata JSON for the Akira RAG pipeline.
...
def _setup_logging(level: str = "INFO") -> logging.Logger:
    ...
    return logging.getLogger("akira.embeddings")
...
def _scrape_url(url: str, timeout: int = 15, retries: int = 3) -> str:
    ...
    for attempt in range(1, retries + 1):
        try:
            resp = requests.get(url, timeout=timeout, headers={"User-Agent": "Akira/1.0"})
...
def embed_chunks(
    ...
    total = len(texts)
    log.info("Embedding %d chunks (batch=%d) …", total, cfg.batch_size)

    t0 = time.perf_counter()
    
    # Process in smaller segments to provide more frequent progress logging
    # for environments where the built-in tqdm bar is suppressed.
    segment_size = 500
    all_embeddings = []
    
    for i in range(0, total, segment_size):
        end = min(i + segment_size, total)
        segment_texts = texts[i:end]
        
        percent = (i / total) * 100
        log.info("  Progress: %d/%d (%.1f%%) [Segment %d-%d] …", i, total, percent, i, end)
        
        segment_emb = model.encode(
            segment_texts,
            batch_size=cfg.batch_size,
            show_progress_bar=False,
            convert_to_numpy=True,
        ).astype("float32")
        all_embeddings.append(segment_emb)
        
    embeddings = np.vstack(all_embeddings)
    elapsed = time.perf_counter() - t0

    faiss.normalize_L2(embeddings)      # ← required for cosine sim via IndexFlatIP
    log.info("Embedding complete in %.1f s  (%.0f chunks/s)", elapsed, total / elapsed)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        log.info("=" * 60)
        log.info("Akira Embeddings Builder")
